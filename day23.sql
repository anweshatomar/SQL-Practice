...
select o.customer_id, o.product_id,
    count(*) over(partition by o.product_id,o.customer_id) as Product_count
from orders o
join customers c
on o.customer_id = c.customer_id
)

select distinct c.customer_id, c.product_id, p.product_name
from cte c
join products p
on c.product_id=p.product_id
where c.Product_count= (select max(cc.Product_count) from cte cc where cc.customer_id=c.customer_id)
