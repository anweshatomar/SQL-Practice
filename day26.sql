...
with cte as(select count(distinct requester_id,accepter_id) as c1 from RequestAccepted),
cte2 as(select count(distinct sender_id,send_to_id) as c2 from friendrequest)
select 
case
when t1.c1!=0 and t2.c2!=0 then round(t1.c1/t2.c2,2)
else 0
end as accept_rate
from cte2 t2, cte t1;
